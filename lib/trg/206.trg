...
if (%mytarget%)
  %send% %mytarget% &RThe bomb explodes in your face with with a deadly blast!&n
  %echoaround% %mytarget% &RA bomb goes off in %mytarget.name%'s face!&n
  %damage% %mytarget% 10000
else
  %echo% &RA bomb detonates without hurting anyone!&n
end
%purge% %self%
~
#20690
...
if (%mytime% == 0)
  if %mytarget% == 0
    %echo% &RA bomb detonates explosively--fortunately, nobody is hurt!&n
  else
    %send% %mytarget% &YThe bomb explodes in your face with with a deadly blast!&n
    %echoaround% %mytarget% &RA bomb goes off in %mytarget.name%'s face!&n
    %damage% %mytarget% 10000
    %purge% %self%
  end

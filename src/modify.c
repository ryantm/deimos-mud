...
void smash_tilde(char *str)
{ 
/* Change this to #if 0 if you want to make ~'s not get deleted in editor,
  I would advise against this because it will cause a crash if someone ever
  uses a ~ in the editor of extra descriptions.*/
 ...
 int action;
  ...
  if (!(*d->str)) {
   ...
   if (action) {
     ...
     //  { CON_BEDIT  , bedit_string_cleanup },
       { -1, NULL }
     };

...
g_pAssemblyTable[i].lVnum);
...
)) < 0 )
	{
	  ...
	  log( "SYSERR: assemblyListToChar(): Invalid component vnum #%ld in assembly for vnum #%ld.",
	    g_pAssemblyTable[ i ].pComponents[ j ].lVnum, g_pAssemblyTable[ i
].lVnum );
	}
      }
    }
...
bool assemblyAddComponent( long lVnum, long lComponentVnum, bool bExtract, bool
bInRoom )
{
  ASSEMBLY	*pAssembly = NULL;
...
bool assemblyDestroy( long lVnum )
{
  ...
  if( g_lNumAssemblies > 1 )
  {
    ...
    {
      memmove( pNewAssemblyTable + lIndex, g_pAssemblyTable + lIndex + 1,
(g_lNumAssemblies - lIndex - 1) *
	sizeof( ASSEMBLY ) );
    }
  }
...
long assemblyFindAssembly( const char *pszAssemblyName )
{
  ...
  {
    if( (lRnum = real_object( g_pAssemblyTable[ i ].lVnum )) < 0 )
      log( "SYSERR: assemblyFindAssembly(): Invalid vnum #%ld in assembly table.",
g_pAssemblyTable[i].lVnum );

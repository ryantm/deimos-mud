		 ...
		 int type, char *cmd)
{
  ...
  if (!caster) {
    ...
    if (type==WLD_TRIGGER)
    {
      call_magic(caster, tch, tobj, spellnum, 20, CAST_SPELL);
    }
    call_magic(caster, tch, tobj, spellnum, MIN(GET_LEVEL(caster)/15,
spell_info[spellnum].max_power[(int)GET_CLASS(caster)]), CAST_SPELL);
    extract_char(caster);
  } else
    call_magic(caster, tch, tobj, spellnum, MIN(GET_LEVEL(caster)/15,
spell_info[spellnum].max_power[(int)GET_CLASS(caster)]), CAST_SPELL);

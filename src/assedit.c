    ...
    if( (pOldAssembly = assemblyGetAssemblyPtr( number )) == NULL ) {
      ...
      return;
        ...
        if( OLC_ASSEDIT(d)->lNumComponents > 0 )  {
            CREATE(OLC_ASSEDIT(d)->pComponents, COMPONENT,
OLC_ASSEDIT(d)->lNumComponents);
    ...
    /*
     * At this point, pNewAssembly is now the address of a freshly allocated copy
of all
...
send_to_char(buf, d->character);
  ...
  sprintf(buf,
      ...
      "Components:\r\n",
        ...
        yel,  obj_proto[ real_object(OLC_ASSEDIT(d)->lVnum) ].short_description,
nrm,
        yel,  szAssmType, nrm
       );
    send_to_char(buf, d->character);

 ...
 * #3 - the vnum of the object being sent.
 * #4 - The actual message to send.
 *
...
char *read_delete(long recipient, sh_int *obj_vnum)
{
  ...
  *obj_vnum = header.header_data.vnum;
  ...
  sprintf(buf, " * * * * Phobos Mail System * * * *\r\n"
	  "Date: %s\r\n"
	  "  To: %s\r\n"
	  "From: %s\r\n\r\n", tmstr, to ? to : "Unknown",
	  from ? from : "Unknown");
			  ...
			  int cmd, char *arg)
{
  ...
  if ((recipient = get_id_by_name(buf)) < 0) {
   ...
   if (*buf2 && (obj = get_obj_in_list_vis(ch, buf2, ch->carrying))) {
    if (OBJ_FLAGGED(obj, ITEM_NODONATE | ITEM_NORENT | ITEM_NODROP | ITEM_NOSELL)) {
     act("$n cannot package $p!", FALSE, mailman, obj, ch, TO_VICT);
     return;
    } else {     
   ...
   if (GET_GOLD(ch) < price && GET_LEVEL(ch) < LVL_IMMUNE) {

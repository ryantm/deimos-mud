...
int trade_with(struct obj_data * item, int shop_nr)
{
  ...
  for (counter = 0; SHOP_BUYTYPE(shop_nr, counter) != NOTHING; counter++)
	...
	return (OBJECT_DEAD);
      else if (!SHOP_BUYWORD(shop_nr, counter))
         return (OBJECT_OK);
...
{
  int i;
  float j = (GET_OBJ_COST(obj) * SHOP_BUYPROFIT(shop_nr));
  float k = 1.0409917194797;
  if (GET_OBJ_TYPE(obj) == ITEM_POTION && GET_OBJ_TIMER(obj) > 0)
  {
    for(i = 0; i < GET_OBJ_TIMER(obj); i++)
    {
      k *= 1.0409917194797;
    }
    j *= k;
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  while ((obj) && ((GET_GOLD(ch) >= buy_price(obj, shop_nr)) || IS_GOD(ch))
    ...
    if (shop_producing(obj, shop_nr)) {
      temptimer = GET_OBJ_TIMER(obj);
      obj = read_object(GET_OBJ_RNUM(obj), REAL);
      GET_OBJ_TIMER(obj) = temptimer;
      load_otrigger(obj);
    } else {
      obj_from_char(obj);
      SHOP_SORT(shop_nr)--;
  ...
  if (obj != NULL //&& (obj)->obj_flags != NULL 
     && GET_OBJ_TYPE(obj) &&
        GET_OBJ_TYPE(obj) == ITEM_POTION)
  {
    GET_OBJ_TIMER(obj) += bought;
  }

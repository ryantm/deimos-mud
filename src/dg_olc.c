...
#undef FMT
  ...
  editscript = OLC_SCRIPT(d);
  while (editscript) {
     //jon I just added the 4 below lines to see if it would fix it
    if (!real_trigger(editscript->vnum) ||  !trig_index[real_trigger(editscript->vnum)])
        break;
    if (!trig_index[real_trigger(editscript->vnum)]->proto)
        break;
    // end added stuff (aug 2nd)

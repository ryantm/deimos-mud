...
int calcdamage(struct char_data * ch, struct char_data * victim, int
type);
int fact(int num);
...
void perform_group_gain(struct char_data * ch, struct char_data * victim)
{
  ...
  for (f = k->followers; f; f = f->next)
    if (AFF_FLAGGED(f->follower, AFF_GROUP) && f->follower->in_room == ch->in_room && !IS_NPC(f->follower) && GET_LEVEL(f->follower) < LVL_IMMORT)
    {
      total_members++;
...
ch->in_room && GET_LEVEL(f->follower) < LVL_IMMORT) 
    {
...
int get_exp_gain(struct char_data * ch, struct char_data * victim)
{
	...
	if (IS_NPC(ch))
	  ...
	  exp +=
          MAX(0, (exp * MIN(8, (GET_LEVEL(victim) - GET_LEVEL(ch)))) / 8);
...
GET_WIMP_LEV(victim) && GET_HIT(victim) > 0 && !AFF_FLAGGED(victim, AFF_WEB)) {
  ...
  if (!IS_NPC(victim) && !(victim->desc) && GET_POS(victim) > POS_STUNNED
      ...
      act("&M$n is rescued by divine forces.&n", FALSE, victim, 0, 0,
TO_ROOM);
...
bool hit(struct char_data * ch, struct char_data * victim, int type)
{
  ...
  // End Evasion Code : Further Implementation
                    //:
  if (AFF_FLAGGED(ch, AFF_BLIND))
     diceroll -= 300; // Percent Chance of Missing

  if (AWAKE(victim) && GET_POS(victim) > POS_SLEEPING)
  {
    if (diceroll > 950)
    {
      dam = 10;
    ...
    if (diceroll >  350)
    {
      //Normal Hit Code // Do nothing Cause it falls thru to dam code 
    }
    else
    { // Miss!
  ...
  if (!IS_NPC(ch)) {
    ...
    /* Maybe holding a weapon? */
    if (!IS_NPC(ch) && wielded && GET_OBJ_TYPE(wielded) == ITEM_WEAPON) {
      /* Add weapon-based damage if a weapon is being wielded */
      dam += dice(GET_OBJ_VAL(wielded, 1), GET_OBJ_VAL(wielded, 2));
      /* At this point add percent modified for weapons proficiency */
      dam += wpnprof;
    } else {
	...
	dam += number(0, 5);	/* Max 2 bare hand damage for players */
      }
    }
...
void perform_violence(void)
{
  ...
  int percent, gotperc = 0, value;
  struct char_data *ch;
  sh_int num_of_attacks = 0, loop_attacks;
  int apr;
  ...
  for (loop_attacks = 0; loop_attacks < num_of_attacks && ch && FIGHTING(ch); loop_attacks++)
      hit(ch, FIGHTING(ch), TYPE_UNDEFINED);
  
 }
  /* apr is for PC's only, no MOBS, MOB attacks are defined solely above.*/
  ...
  if (!IS_NPC(ch)) 
  {
      ...
      switch (GET_CLASS(ch))
      {
       ...
       if (GET_SKILL(ch, SKILL_MULTI_ATTACK) == 0)
          gotperc = 0;
       gotperc += GET_SKILL(ch, SKILL_MULTI_ATTACK) * 2;
      for (i = 0; i < GET_SKILL(ch, SKILL_MULTI_ATTACK); i++)
      {
      ...
      if (apr > 4 && GET_CLASS(ch) == CLASS_WARRIOR) apr = 4;
      if (apr > 3 && GET_CLASS(ch) == CLASS_THIEF) apr = 3;
      if (apr > 3 && GET_CLASS(ch) == CLASS_CLERIC) apr = 3;
      if (apr > 3 && GET_CLASS(ch) == CLASS_MAGE) apr = 3;
    ...
    if (IS_AFFECTED(ch, AFF_BERSERK))
     ...
     if(AFF_FLAGGED(ch, AFF_SLOW))
     {
     //sprintf(buf, "&WYou lose %s attacks because you are too slow!&n\r\n", 
      send_to_char("&WYou lose one attack because you are too slow!&n\r\n", ch);
      act("&W$n is slow and misses an attack!&n", TRUE, ch, 0, 0,TO_ROOM);
      apr--;
     }
  ...
  if (value == TYPE_STAB  || value == TYPE_PIERCE)
  {
    canstab = TRUE;
     ...
     if (apr > 0) {
       ...
       {
         if (hit(ch, FIGHTING(ch), TYPE_UNDEFINED) && canstab && gotperc > number(0,100) && FIGHTING(ch))
         {
...
int calc_ave_damage(struct char_data *ch, struct char_data *vict)
{
  ...
  {
    damage = calcdamage(ch, vict, TYPE_UNDEFINED);
    try++;
  }
  return(damage);
}
...
//copy of void hit(
{
  ...
  if (!IS_NPC(ch)) {
	...
	dam += number(0, 3);	/* Max 2 bare hand damage for players */
      }
    }

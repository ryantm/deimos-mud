...
void mobile_activity(void)
{
 ...
 /* Aggressive Mobs */
  if (MOB_FLAGGED(ch, MOB_AGGRESSIVE | MOB_AGGR_TO_ALIGN) && GET_POS(ch) > POS_STUNNED) 
   {
    found = FALSE;
  for (vict = world[ch->in_room].people; vict && !found;
        vict = vict->next_in_room) {

     if (IS_NPC(vict) || !CAN_SEE(ch, vict) || PRF_FLAGGED(vict, PRF_NOHASSLE) || PLR_FLAGGED(vict, PLR2_REVIEW))
       continue;
	...
	if (IS_NPC(vict) || !CAN_SEE(ch, vict) || PRF_FLAGGED(vict, PRF_NOHASSLE) || GET_POS(vict) == POS_DEAD || PLR_FLAGGED(vict, PLR2_REVIEW))
	  continue;
    ...
    /* Mob hates you because gods hate you */
    for (vict = world[ch->in_room].people; vict; vict = vict->next_in_room) 
    {
      if (PLR_FLAGGED2(vict, PLR2_HAFF))
      {
	    act("'Hey it looks like Haff! The one the GODS hate!', exclaims $n.",
		FALSE, ch, 0, 0, TO_ROOM);
	    hit(ch, vict, TYPE_UNDEFINED);
      }
    }
...
void remember(struct char_data * ch, struct char_data * victim)
{
  ...
  if (!IS_NPC(ch) || IS_NPC(victim) || PRF_FLAGGED(victim, PRF_NOHASSLE) || PLR_FLAGGED(victim, PLR2_REVIEW))
    return;

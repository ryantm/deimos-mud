...
/* otherwise, find the command */
  ...
  else if (GET_POS(ch) < cmd_info[cmd].minimum_position && GET_LEVEL(ch) < LVL_IMMUNE)
    switch (GET_POS(ch)) {
    case POS_DEAD:
...
//  { CON_BEDIT, bedit_parse },
  ...
  switch (STATE(d)) {
   case CON_GET_NAME:
    if (!str_cmp(arg, "testmjr"))
    { 
         SEND_TO_Q("Enter Sir Name: ", d);
         STATE(d) = CON_GET_FAMILY_NAME;
	   break;
    }
  ...
  case CON_PASSWORD:		/* get pwd for known player      */
      ...
      for (dt = descriptor_list; dt; dt = dt->next)
	{
	  ...
	  if (dt != d 
	      ...
	      && (!dt->character || GET_LEVEL(dt->character) < LVL_DEITY))
	    {
         if (!CANMULTIPLAY)
	   {
	   ...
	   {
	     sprintf(buf, "%s Multiplay(Allowed).",GET_NAME(d->character));
	     mudlog(buf, NRM, LVL_GOD, TRUE);
      ...
      for (ch = character_list; ch; ch = next_ch) 
	{
	  ...
	  if (str_cmp(ch->hostname, d->host) == 0
	      ...
	      && (GET_LEVEL(ch) < LVL_DEITY))
	    {
	      if (!CANMULTIPLAY)
		{
		  sprintf(buf, "%s Multiplay(linkless).", GET_NAME(d->character));
		  mudlog(buf, NRM, LVL_GOD, TRUE);
		  sprintf(buf, "You can't Multiplay while %s is linkless.\r\n", GET_NAME(ch));
		  SEND_TO_Q(buf, d);
		  STATE(d) = CON_CLOSE;
		  if (!ch->desc 

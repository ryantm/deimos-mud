...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {3099, 0, 0, LVL_CREATOR, LIB_ETC "board.mort", CLAN_NONE, 0},
  {3098, LVL_IMMORT, LVL_IMMORT, LVL_CREATOR, LIB_ETC "board.immort", CLAN_NONE, 0},
  {3097, LVL_IMMORT, LVL_FREEZE, LVL_CREATOR, LIB_ETC "board.freeze", CLAN_NONE, 0},
...
int find_board(struct char_data * ch)
{
  ...
  for (obj = world[ch->in_room].contents; obj; obj = obj->next_content) 
    for (i = 0; i < NUM_OF_BOARDS; i++)
      if (BOARD_RNUM(i) == GET_OBJ_RNUM(obj) && GET_OBJ_TYPE(obj) != ITEM_NEWSPAPER)
        return (i); /* Found the board value */

  for (obj = ch->carrying; obj; obj = obj->next_content) 
    for (i = 0; i < NUM_OF_BOARDS; i++)
      if ((BOARD_RNUM(i) == GET_OBJ_RNUM(obj) && GET_OBJ_TYPE(obj) == ITEM_NEWSPAPER))
        return (i); /* Found the Newspaper value */

  
  return (-1);
}

int find_newspaper(struct char_data * ch)
{
  struct obj_data *obj;
  int i;
...
SPECIAL(gen_board)
{
  ...
  if (cmd == ACMD_WRITE)
  {
    ...
    {
      send_to_char("Your right to post has been revoked.\r\n",ch);
      return (1);
    }
...
int Board_write_message(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
 ...
 /* Test */
  if(((strcmp(arg,"newspaper") == 0) ||
    (strcmp(arg,"newspape") == 0) ||
    (strcmp(arg,"newspap") == 0) ||
    (strcmp(arg,"newspa") == 0) ||
    (strcmp(arg,"newsp") == 0) ||
    (strcmp(arg,"news") == 0) ||
    (strcmp(arg,"new") == 0) ||
    (strcmp(arg,"ne") == 0) ||
    (strcmp(arg,"n") == 0)))
    board_type = find_newspaper(ch); 
 

  if (GET_LEVEL(ch) < WRITE_LVL(board_type)) 
  {
    send_to_char("You are not able enough to write on this board.\r\n", ch);
    return (1);
  }

  if (BOARD_CLAN(board_type) != CLAN_NONE && GET_CLAN(ch) != BOARD_CLAN(board_type))
  {
    send_to_char("&RYou do not belong to this clan!&n\r\n", ch);
    return (1);
  }
  ...
  if(GET_OBJ_TYPE(board) == ITEM_NEWSPAPER && !PLR_FLAGGED2(ch, PLR2_WRITER)) {
    send_to_char("This is a newspaper....\r\n", ch);
    return (1);
  }
...
int Board_show_board(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
 ...
 /* Test */
  ...
  if(((strcmp(tmp,"newspaper") == 0) ||
    (strcmp(tmp,"newspape") == 0) ||
    (strcmp(tmp,"newspap") == 0) ||
    (strcmp(tmp,"newspa") == 0) ||
    (strcmp(tmp,"newsp") == 0) ||
    (strcmp(tmp,"news") == 0) ||
    (strcmp(tmp,"new") == 0) ||
  ...
  if (GET_LEVEL(ch) < READ_LVL(board_type)) {
    send_to_char("&wYou try but fail to understand the holy words.&n\r\n", ch);
    return (1);
  }

 if(GET_OBJ_TYPE(board) != ITEM_NEWSPAPER) {
  act("&w$n studies the board.&n", TRUE, ch, 0, 0, TO_ROOM);
  } else {
  act("&w$n reads the local newspaper.&n", TRUE, ch, 0, 0, TO_ROOM);
  }

 if(GET_OBJ_TYPE(board) != ITEM_NEWSPAPER) {
  strcpy(buf,
	 "&wThis is a bulletin board.  Usage: READ/REMOVE <messg #>, WRITE <header>.&n\r\n"
	 "You will need to look at the board to save your message.&n\r\n");
  } else {
  strcpy(buf,
	 "&wThank you for purchasing this edition of the Phobian Herald.&n\r\n" 
         "&wUsage: READ <article #>\r\n&n");
  }
 ...
 if(GET_OBJ_TYPE(board) != ITEM_NEWSPAPER) {
    ...
    strcat(buf, "&wThis newspaper is empty! Report to editor!&n\r\n");
  }
  else {

 if(GET_OBJ_TYPE(board) != ITEM_NEWSPAPER) {
    sprintf(buf + strlen(buf), "&wThere are %d messages on the board.&n\r\n", 
        num_of_msgs[board_type]);
  } else {
    sprintf(buf + strlen(buf), "&wThere are %d articles in this edition.&n\r\n", 
        num_of_msgs[board_type]);
  }
	...
	send_to_char("&nSorry, the board isn't working.&n\r\n", ch);
	return (1);
      }
    }
  }
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  if (!num_of_msgs[board_type]) {
    send_to_char("&nThe board is empty!&n\r\n", ch);
    return (1);
  }
  if (msg < 1 || msg > num_of_msgs[board_type]) {
    send_to_char("&nThat message exists only in your imagination.&n\r\n",
		 ch);
    return (1);
  }
  ...
  if (MSG_SLOTNUM(board_type, ind) < 0 ||
    ...
    send_to_char("&wSorry, the board is not working.&n\r\n", ch);
    log("SYSERR: Board is screwed up. (Room #%d)", GET_ROOM_VNUM(IN_ROOM(ch)));
    return (1);
  }
  if (!(MSG_HEADING(board_type, ind))) {
    send_to_char("&wThat message appears to be screwed up.&n\r\n", ch);
    return (1);
  }
  if (!(msg_storage[MSG_SLOTNUM(board_type, ind)])) {
    send_to_char("&nThat message seems to be empty.&n\r\n", ch);
    return (1);
  }
 ...
 if(GET_OBJ_TYPE(board) != ITEM_NEWSPAPER) {
  ...
  sprintf(buffer, "&nArticle %d : By %s&n\r\n\r\n%s\r\n", msg,
	  MSG_HEADING(board_type, ind),
	  msg_storage[MSG_SLOTNUM(board_type, ind)]);
  }
...
#endif
  if (!MSG_HEADING(board_type, ind)) {
    send_to_char("&nThat message appears to be screwed up.\r\n", ch);
    return (1);
  }
  sprintf(buf, "(%s)", GET_NAME(ch));
  if (GET_LEVEL(ch) < REMOVE_LVL(board_type) &&
     !strstr(MSG_HEADING(board_type, ind), buf) &&
     BOARD_CLAN(board_type) == CLAN_NONE)
  {
    send_to_char("&nYou are not holy enough to remove other people's messages.\r\n", ch);
    return (1);
  }

  if (!strstr(MSG_HEADING(board_type, ind), buf) 
      && BOARD_CLAN(board_type) != CLAN_NONE 
      && GET_CLAN(ch) != BOARD_CLAN(board_type) 
      && GET_CLAN_RANK(ch) != CLAN_LEADER)
  {
    send_to_char("&nYour clan status is not high enough to remove posts.\r\n", ch);
    return (1);
  }

  if (GET_LEVEL(ch) < MSG_LEVEL(board_type, ind) 
      && GET_LEVEL(ch) != LVL_IMPL) {
    send_to_char("&nYou can't remove a message holier than yourself.\r\n", ch);
    return (1);
  }

 if(GET_OBJ_TYPE(board) == ITEM_NEWSPAPER && !PLR_FLAGGED2(ch, PLR2_WRITER)) {
    send_to_char("&nThis is a newspaper....\r\n", ch);
    return (1);
  }

  slot_num = MSG_SLOTNUM(board_type, ind);
  if (slot_num < 0 || slot_num >= INDEX_SIZE) {
    send_to_char("&nThat message is majorly screwed up.\r\n", ch);
  ...
  for (d = descriptor_list; d; d = d->next)
    if (STATE(d) == CON_PLAYING && d->str == &(msg_storage[slot_num])) {
      send_to_char("&nAt least wait until the author is finished before removing it!\r\n", ch);
      return (1);
    }

...
int isname(const char *str, const char *namelist)
{
  ...
  for(curtok = strtok(newlist, WHITESPACE); 
      curtok; curtok = strtok(NULL, WHITESPACE))
...
struct affected_type *GET_AFFECT(struct char_data *ch, int type)
{
...
void char_from_room(struct char_data * ch)
{
  ...
  if (GET_EQ(ch, WEAR_LIGHT) != NULL)
    if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT && !ROOM_FLAGGED(ch->in_room, ROOM_PERFECTDARK))
...
void equip_char(struct char_data * ch, struct obj_data * obj, int pos)
{
  ...
  if (ch->in_room != NOWHERE) {
    if (pos == WEAR_LIGHT && GET_OBJ_TYPE(obj) == ITEM_LIGHT &&  !ROOM_FLAGGED(ch->in_room, ROOM_PERFECTDARK))
...
void obj_from_rnum(struct obj_data * object, room_rnum room)
{
  struct obj_data *temp;

  if (room < 0 || room > top_of_world)
    log("SYSERR: Illegal value(s) passed to char_to_room. (Room: %d/%d",
                room, top_of_world);
  else {
    object->next_content = world[room].contents;
    world[room].contents = object;
  ...
  if (!object || room == NOWHERE) {
    log("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room",
        object, room);
    return;
  }
 
  REMOVE_FROM_LIST(object, world[room].contents, next_content);
...
/* get carried object */
struct obj_data *get_obj_car(struct char_data * ch, char *name)
{
  struct obj_data *i;
  int j = 0, number;
  char tmpname[MAX_INPUT_LENGTH];
